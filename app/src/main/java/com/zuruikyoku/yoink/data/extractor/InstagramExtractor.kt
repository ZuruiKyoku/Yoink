...
// Disabled for now: even with the link-preview-UA workaround below, Instagram only ever
// yields a static cover image for videos/carousels (real og:video is gone, and there's no
// anonymous path to the real video/carousel data), and that cover image itself has a
// non-functional play-button icon baked into it - which reads as a broken download rather
// than an inherent platform limitation. ExtractorRegistry no longer wires this extractor up,
// and MainViewModel shows a clear "Instagram isn't supported" notice instead of calling it.
// The implementation (imports included) is kept commented out below, not deleted, in case
// Instagram's anonymous surface improves or a future session adds a logged-in-session
// approach - re-enabling means uncommenting this file and the registry entry. (Line comments
// rather than a /* */ block: this file's Accept header value contains a literal "*/", which
// closes a block comment early regardless of it being inside a string.)
